...
namespace ProyectoBiblioteca.Logica
{
    public class PrestamoLogica
    {
        ...
        public List<Prestamo> Listar(int idestadoprestamo, int idpersona)
        {
            ...
            using (SqlConnection oConexion = new SqlConnection(Conexion.CN))
            {
                try
                {
                    ...
                    query.AppendLine("select p.IdPrestamo,ep.IdEstadoPrestamo,ep.Descripcion,pe.Codigo,pe.Nombre,pe.Apellido,pe.Carrera,pe.Matricula,li.Titulo,convert(char(10),p.FechaDevolucion,111)[FechaDevolucion],convert(char(10),p.FechaConfirmacionDevolucion,111)[FechaConfirmacionDevolucion],p.EstadoEntregado,p.EstadoRecibido from prestamo p");
                    ...
                    using (SqlDataReader dr = cmd.ExecuteReader())
                    {
                        while (dr.Read())
                        {
                            Lista.Add(new Prestamo()
                            {
                                ...
                                oPersona = new Persona() { Codigo = dr["Codigo"].ToString(), Nombre = dr["Nombre"].ToString(), Apellido = dr["Apellido"].ToString(), Carrera = dr["Carrera"].ToString(), Matricula = dr["Matricula"].ToString() },
        ...
        public bool ActualizarEstadoIdLibroPrestamo(int idprestamo, bool estado)
        {
            ...
            using (SqlConnection oConexion = new SqlConnection(Conexion.CN))
            {
                ...
                {
                    StringBuilder query = new StringBuilder();
                    query.AppendLine("select IdLibro from prestamo");
                    query.AppendLine("where IdPrestamo = @idprestamo");
                    ...
                    using (SqlDataReader rd = cmd.ExecuteReader())
                    {
                        while (rd.Read())
                        {
                            idLibro = Convert.ToInt32(rd["IdLibro"]);
                        }
                    }
                    respuesta = ActualizarEstadoLibro(idLibro, estado);
                }
                catch (Exception ex)
                {
                    respuesta = false;
        ...
        public bool ActualizarEstadoLibro(int idlibro, bool estado)
        {
            bool respuesta = true;
            using (SqlConnection oConexion = new SqlConnection(Conexion.CN))
            {
                ...
                {
                    StringBuilder query = new StringBuilder();
                    query.AppendLine("update libro set Estado = @estado");
                    query.AppendLine("where IdLibro = @idlibro");
        ...
        public int CountPrestamosPendientes(int idpersona)
        {
            int respuesta = 0;
            using (SqlConnection oConexion = new SqlConnection(Conexion.CN))
            {
                ...
                {
                    StringBuilder query = new StringBuilder();
                    query.AppendLine("select count(*) from prestamo");
                    query.AppendLine("where IdPersona = @idpersona and IdEstadoPrestamo = 1");
