...
namespace ProyectoBiblioteca.Controllers
{
    public class BibliotecaController : Controller
    {
        ...
        [HttpGet]
        public JsonResult ListarLibroActivo()
        {
            List<Libro> oLista = new List<Libro>();
            List<Libro> listaLibro = new List<Libro>();

            oLista = LibroLogica.Instancia.Listar();
            foreach (var libro in oLista)
            {
        ...
        public JsonResult GuardarLibro(string objeto, HttpPostedFileBase imagenArchivo)
        {
            ...
            try
            {
                ...
                if (oLibro.IdLibro == 0)
                {
                    int ejemplares = Int32.Parse(oLibro.Ejemplar);
                    for (int i = 0; i < ejemplares; i++)
                    {
                        string codigoEjemplar = ObtainCodigoEjemplar(oLibro.Titulo, i);
                        oLibro.Ejemplar = codigoEjemplar;
        ...
        public JsonResult ListarPersona()
        {
            ...
            foreach (var persona in oLista)
            {
                if (persona.oTipoPersona.IdTipoPersona == 3)
                {
            ...
            {
                oLista.Remove(persona);
            }
            return Json(new { data = oLista }, JsonRequestBehavior.AllowGet);
        }
        [HttpGet]
        public JsonResult ListarLectores()
        {
            List<Persona> oLista = new List<Persona>();
            oLista = PersonaLogica.Instancia.Listar();
            return Json(new { data = oLista }, JsonRequestBehavior.AllowGet);
        ...
        [HttpGet]
        public JsonResult ListarLector()
        {
            List<Persona> oLista = new List<Persona>();
            List<Persona> listPersona = new List<Persona>();
            oLista = PersonaLogica.Instancia.Listar();
            foreach (var persona in oLista)
            {
                if (persona.Estado == false && persona.oTipoPersona.IdTipoPersona == 3)
                {
                    listPersona.Add(persona);
                    continue;
                }
        ...
        public JsonResult GuardarPersona(Persona objeto)
        {
            ...
            objeto.Matricula = objeto.Matricula == null ? "" : objeto.Matricula;
            objeto.Carrera = objeto.Carrera == null ? "" : objeto.Carrera;
        ...
        public static string ObtainCodigoEjemplar(string input, int numeroEjemplar)
        {
            string newText = "";
            int tam_cadena = input.Length;
            if (string.IsNullOrEmpty(input))
            {
                return input;
            };
            newText = input.Substring(0,3);
            newText += input.Substring((tam_cadena - 1), 1);
            numeroEjemplar = numeroEjemplar + 1;
            newText += "-" + numeroEjemplar;
            return newText;
        }
    }
